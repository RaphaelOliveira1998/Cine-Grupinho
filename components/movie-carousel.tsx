...
export function MovieCarousel({ movies }: MovieCarouselProps) {
  ...
  return (
    <>
          ...
          {movies.map((movie) => {
            ...
            return (
              <article
                ...
                className="group relative min-w-[180px] snap-start overflow-hidden rounded-3xl border border-white/10 bg-slate-950/80 sm:min-w-[210px] cursor-pointer hover:border-violet-400/60 transition-colors"
              >
                <div className="relative aspect-[2/3] bg-slate-900">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                    <span className="rounded-full bg-black/60 px-4 py-2 text-sm font-semibold text-white">▶ Ver trailer</span>
                  </div>
                </div>
      ...
      {selectedId !== null && (
        <MovieDetailModal tmdbId={selectedId} onClose={() => setSelectedId(null)} />
      )}
    </>
  )
}
